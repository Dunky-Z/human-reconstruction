...
int main()
{
	...
	mesh.read((DATASET_PATH + "R5.obj").c_str());
	...
	meshio::ReadObj((DATASET_PATH + "R5.obj").c_str(), verts, facets);
	one_measure.resize(19, 1);
	one_measure = measure.CalcMeasure(control_points, verts, facets);
	cout << one_measure << endl;
	...
	std::vector<std::vector<int>>& point_idx)
{
	...
	{
		if (i == 0 || i == 5 || i == 6 || i == 7 || i == 8 || i == 11 || i == 12 || i == 15)
		{
			edge.push_back(point_idx[i].size() - 1);
		}
		else
		{
			edge.push_back(point_idx[i].size());
		}
	}
}
	...
	std::vector<Tri>& triplets_C)
{
	...
	for (int i = 0; i < num_measure; ++i)
	{
		...
		for (int j = 0; j < edge[i]; ++j)
		{
			...
			int edge_1;
			//i == 0身高信息
			if ((i == 0 || i == 5 || i == 6 || i == 7 || i == 8 || i == 11 || i == 12 || i == 15) && j + 1 <= edge[i])
			{
				int aaa = point_idx[i].size();
