...
public:
	...
	void ConstructCoefficientMatrixBottom(Eigen::SparseMatrix<double>& A, std::vector<std::vector<int>>& point_idx, const Eigen::Matrix3Xd &vertices, const Eigen::MatrixXd& measurements,
	...
	void ConstructCoefficientMatrix(Eigen::SparseMatrix<double>& A, const Eigen::SparseMatrix<double>& L, const Eigen::SparseMatrix<double>& C, std::vector<Tri>& triplets);
protected:
private:
	int M_NUM;
	const double step = 1;
